...
//I have enabled the flag to calculate the L2,1 norm, to disable it please set the flag to false.
// Also commenting all the matrix printing as for larger values printing takes time.


// calculating the product of L and U matrix for the L2 norm calculation
...
// this function applies permutation to the matrix
//it return a vector<vector<double>> that is the permuted matrix according to the permutation vector pi
...
// Function to calculate the L21 norm of a square matrix.
...
// Function to initialize a square matrix with random values in the range [0.5, 3.0] using a 
// specified seed for reproducibility.
void intialize_matrix(vector<vector<double>>& matrix, unsigned int seed){

    std::mt19937 generator(seed);
    std::uniform_real_distribution<> dist(0.5, 3.0); //uniform real distribution between 0.5 and 3.0.
...
void PA_LU_Decomposition(vector<vector<double> >& matrix, int n, vector<double> &pi, vector<vector<double> >& A, bool l2_norm_flag) {
    ...
    auto start = high_resolution_clock::now(); // defining the clock to count the time for LU decomposition

    vector<vector<double> > L(n, vector<double>(n, 0)); //initializing L matrix with all 0
    vector<vector<double> > U(n, vector<double>(n, 0)); //initializing U matrix with all 0

    pi.resize(n);
    
    // Initialize the permutation vector pi with consecutive integers starting from 0 to n-1.
    for (int i = 0; i < n; i++) {
        pi[i] = i;
    }
    ...
    // setting all diagonal values of L matrix to 1
    for (int i = 0; i < n; i++) {
        L[i][i] = 1.0;
    }
    ...
    // LU code according to given pseudo code in the assignment description
    for(int k = 0; k < n; k++){
        double max_val = 0.0;
        int k_prime;
        int i;
...
int main(int argc, char **argv)
{
  ...
  bool l2_norm_flag = true;   //flag to toggle checking l2 norm. change it to false to disable l2 norm calculation
